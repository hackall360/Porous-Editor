...
function parseXmlPlist(text: string): UnityParseResult {
  ...
  const dictPattern = /<key>([^<]+)<\/key>\s*<(string|integer|real|true|false)\b[^>]*>(?:([^<]*)<\/\2>)?/g;
  ...
  return {
    ...
    meta: {
      ...
      valueTypes: Object.fromEntries(
        Object.entries(result).map(([key, value]) => [key, inferUnityType(value)])
      ),
    },
  };
}
...
export class UnityParser extends BaseParser<ArrayBuffer, UnityParseResult> {
  ...
  async parse(input: ArrayBuffer, fileName: string): Promise<ParseResult<UnityParseResult>> {
    const startTime = performance.now();
    let data: UnityParseResult;
    let format: UnityInputFormat;
    ...
    try {
      ...
      if (text.trim().startsWith('<?xml') && text.includes('<!DOCTYPE plist')) {
        ...
        if (bytes.length >= 6 && bytes[0] === 0x62 && bytes[1] === 0x70 && bytes[2] === 0x6c && bytes[3] === 0x69 && bytes[4] === 0x73 && bytes[5] === 0x74) {
          throw new Error('Binary plist format is not supported. Please convert to XML plist.');
        }
